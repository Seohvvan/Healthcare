...
- **집계·하드룰은 LLM이 아닌 코드로**: 제외 기준 확정 위반 1건 = excluded,
  그 외에는 **선정 충족률 + 근거 있는 제외 해제 비율(가중치 w, `(1+w)`로 정규화)
  − unknown 페널티**로 점수화. 제외 기준이 없는 시험의 해제 비율은 공허참으로
  1.0 — ELIGIBLE 라벨이 빈 제외 목록을 다루는 방식과 같다.
  판정 근거(evidence)를 인용하지 않은 선정 MET·제외 NOT MET은 코드가 UNKNOWN으로
  되돌린다: 근거 없는 추론만으로는 점수도 ELIGIBLE 라벨도 얻지 못하고 미확정으로
  남아 질문 대상이 된다.
